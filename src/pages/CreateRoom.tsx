...
export default function CreateRoom() {
  ...
  const handleContinuar = () => {
    navigate('/room-code', { state: { tematica, dificultad, tiempo } });
  };
      ...
      <div style={{
        ...
        <div style={{ width: '100%', maxWidth: '420px', position: 'relative', zIndex: 2 }}>
          ...
          <div style={{
            ...
            <p style={{ fontSize: '13px', color: 'rgba(180,255,205,0.75)', margin: 0, fontFamily: "'Inter', sans-serif" }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {TEMATICAS.map(t => (
                  <button
                    key={t.id}
                    className={`option-btn${tematica === t.id ? ' active' : ''}`}
                    onClick={() => setTematica(t.id)}
                    style={{ flex: 'none' }}
                  >
                    {t.label}
                  </button>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                {DIFICULTADES.map(d => (
                  <button
                    key={d.id}
                    className={`option-btn${dificultad === d.id ? ' active' : ''}`}
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                {TIEMPOS.map(t => (
                  <button
                    key={t}
                    className={`option-btn${tiempo === t ? ' active' : ''}`}
                    onClick={() => setTiempo(t)}
                  >
                    {t}s
                  </button>
